    ...
    public class Coord2_BranchOfficerController : Controller
    {
        ...
        public async Task<IActionResult> UpdateVcReferenceByDH()
        {
            try
            {
                ...
                if (!int.TryParse(id, out int proposalId))
                    return BadRequest("Invalid Proposal ID.");

                var model = new VcReferenceReceivedForUpdate
                {
                    masterReferenceId = Int64.Parse(id),
                    officerId= 0
                    //PendingWith = "CVC_COORD2_DH",
                    //UpdateBy = username,
                    //UpdatedBy_SessionId = HttpContext.Session.Id,
                    //UpdatedBy_IP = HttpContext.Connection.RemoteIpAddress?.ToString() ?? "Unknown"
                };
                ...
                var requestUrl = $"{BaseUrl}Coord2_DealingHand/ForwardProposalTo_BranchesAndMinistry";
                ...
                var result = JsonConvert.DeserializeObject<ForwardProposalResponse>(responseJson);
                ...
                Console.WriteLine("API returned success=false or invalid structure.");
                return Ok(0);
            }
            catch (Exception ex)
            {
        ...
        public class ForwardProposalResponse
        {
